...
if (count($teams_array) <= 4) {
    foreach ($teams_array as $num => $team_data) {
        ...
        $htmlOutput .= '<span>' . esc_html($team_data['name']) . '</span>';
        $htmlOutput .= '<div class="alive">';
        if ($team_data['alive'] == 0) {
            $htmlOutput .= '<img src="http://localhost/livepoint/wp-content/plugins/livePoint/img/helmet2.svg" alt="">';
        } else {
            for ($i = 1; $i <= $team_data['alive']; $i++) {
                $htmlOutput .= '<img src="' . esc_url($alive_icon) . '" alt="">';
            }
        }
        $htmlOutput .= '</div>';
        $htmlOutput .= '</div>';
